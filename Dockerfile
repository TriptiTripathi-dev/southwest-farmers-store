...
# This links the storage folder, caches config/routes/views (built here, not
# at image build, so they use Railway's variables; if any cache step fails,
# all caches are cleared and the app runs uncached as before), AND starts the
# server with PHP_CLI_SERVER_WORKERS workers.
EXPOSE 8080
CMD sh -c "php artisan storage:link && { php artisan config:cache && php artisan route:cache && php artisan view:cache || php artisan optimize:clear || true; } && exec php artisan serve --host=0.0.0.0 --port=$PORT --no-reload"