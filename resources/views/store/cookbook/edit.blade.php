<x-app-layout title="Edit Recipe">
    <div class="content-wrapper">
        <div class="container-fluid px-3 px-md-4 py-4">
            ...
            <div class="mb-4">
                <nav class="mb-1">
                    <ol class="breadcrumb mb-0" style="font-size: 0.85rem;">
                        ...
                        <li class="breadcrumb-item text-muted">Edit Recipe</li>
                    </ol>
                </nav>
                <h4 class="fw-bold mb-0 text-dark">Edit Recipe</h4>
            </div>
            ...
            <form action="{{ route('store.cookbook.update', $recipe->id) }}" method="POST">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select name="menu_item_id" class="form-select" required>
                                    ...
                                    @foreach($menuItems as $item)
                                        <option value="{{ $item->id }}" @selected(old('menu_item_id', $recipe->menu_item_id) == $item->id)>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" min="0" name="prep_time_minutes" class="form-control" value="{{ old('prep_time_minutes', $recipe->prep_time_minutes) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-semibold">Cook Time (min)</label>
                                <input type="number" min="0" name="cook_time_minutes" class="form-control" value="{{ old('cook_time_minutes', $recipe->cook_time_minutes) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-semibold">Yield Quantity</label>
                                <input type="number" min="0" step="0.01" name="yield_quantity" class="form-control" value="{{ old('yield_quantity', $recipe->yield_quantity) }}">
                            </div>
                            <div class="col-md-3">
                                <label class="form-label fw-semibold">Yield Unit</label>
                                <input type="text" name="yield_unit" class="form-control" placeholder="e.g. servings" value="{{ old('yield_unit', $recipe->yield_unit) }}">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary fw-bold px-4">Update Recipe</button>
                    <a href="{{ route('store.cookbook.show', $recipe->id) }}" class="btn btn-outline-secondary px-4">Cancel</a>
                </div>
            </form>
    ...
    <script>
        (function () {
            ...
            const existingSteps = @json($recipe->steps->map(fn($s) => ['instruction' => $s->instruction, 'timer_minutes' => $s->timer_minutes]));
            ...
            existingSteps.forEach(s => addStep(s.instruction, s.timer_minutes ?? ''));
            renumber();
        })();
    </script>
</x-app-layout>
