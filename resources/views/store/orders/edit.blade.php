        ...
        <form action="{{ route('store.orders.update', $order->id) }}" method="POST">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    ...
                    <p class="text-muted small mb-0 mt-1">Status: <span class="badge bg-warning text-dark">{{ ucfirst($order->status) }}</span></p>
                </div>
                <div class="d-flex gap-2">
                    <a href="{{ route('store.orders.show', $order->id) }}" class="btn btn-light fw-bold px-4 rounded-pill border shadow-sm">Cancel</a>
            ...
            <div class="row">
                                ...
                                <table class="table align-middle mb-0">
                                    <thead class="bg-light">
                                        <tr>
                                            ...
                                            <th class="py-3 text-center text-muted small fw-bold" style="width: 150px;">QUANTITY</th>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="ps-4 py-3">
                                                ...
                                                <div class="text-muted small font-monospace">UPC: {{ $item->product->sku }}</div>
                                            </td>
                                            ...
                                            <td class="pe-4 text-end fw-bold">₹<span class="item-subtotal">{{ number_format($item->quantity * $item->unit_cost, 2) }}</span></td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                                    <tfoot class="bg-light fw-bold text-dark">
                                        <tr>
                                            ...
                                            <td class="pe-4 text-end fs-5">₹<span id="grandTotal">{{ number_format($order->total_amount, 2) }}</span></td>
                                        </tr>
                                    </tfoot>
                                </table>
                            </div>
                ...
                <div class="col-lg-4">
                    <div class="card border-0 shadow-sm rounded-4">
                        <div class="card-header bg-white border-bottom p-3">
                            <h6 class="fw-bold m-0 text-primary"><i class="mdi mdi-shield-check me-2"></i>Manager Approval Required</h6>
                        </div>
                        <div class="card-body p-4">
                            <div class="mb-3">
                                ...
                                <div class="form-text small mt-2">
                                    <i class="mdi mdi-information-outline me-1"></i> These changes will be logged for audit purposes.
                                </div>
                            </div>

                            <div class="alert alert-warning border-0 rounded-3 mb-0 small">
                                <i class="mdi mdi-alert-circle-outline me-2"></i> Quantity edits after initial generation are restricted to manager-level authorization and require an audit trail.
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        $('.item-qty').on('input change', function() {
            ...
            $(this).closest('tr').find('.item-subtotal').text(subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 }));
