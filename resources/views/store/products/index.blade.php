                ...
                <div class="d-flex flex-wrap gap-2">
                    ...
                    @if (Auth::user()->hasPermission('create_product'))
                        <a href="{{ route('store.products.create') }}"
                            class="btn btn-primary shadow-sm fw-bold d-flex align-items-center">
                        ...
                        <div class="col-12 col-md-4">
                            <div class="input-group shadow-sm">
                                <span class="input-group-text bg-light text-muted px-3"><i
                                        class="mdi mdi-filter-variant"></i></span>
                                <select name="type" class="form-select bg-light py-2" onchange="this.form.submit()">
                                    <option value="">All Product Types</option>
                                    <option value="warehouse" {{ request('type') == 'warehouse' ? 'selected' : '' }}>
                                        Warehouse (Global)</option>
                                    <option value="store" {{ request('type') == 'store' ? 'selected' : '' }}>My Store
                                        (Local)</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <table class="table table-hover align-middle mb-0 text-nowrap">
                            <thead class="bg-light">
                                <tr>
                                    ...
                                    <th class="py-3 text-muted small fw-bold text-uppercase"
                                        style="letter-spacing: 0.5px;">Subcategory</th>
                                    <th class="py-3 text-muted small fw-bold text-uppercase"
                                        style="letter-spacing: 0.5px;">Type</th>
                                    <th class="py-3 text-muted small fw-bold text-uppercase"
                                        style="letter-spacing: 0.5px;">Price</th>
                                    <th class="py-3 text-muted small fw-bold text-uppercase text-center"
                                        style="letter-spacing: 0.5px;">Status</th>
                                    <th class="pe-4 py-3 text-end text-muted small fw-bold text-uppercase"
                                ...
                                @forelse($products as $item)
                                    <tr>
                                                    ...
                                                    <div class="mt-1 d-flex gap-2">
                                                        @if ($item->upc)
                                                            <span class="badge bg-light text-muted border px-2 py-1">
                                                                <i class="mdi mdi-barcode-scan me-1"></i> UPC:
                                                                {{ $item->upc }}
                                                            </span>
                                                        @endif
                                                    </div>
                                        ...
                                        <td class="align-middle">
                                            ...
                                            @if ($barcode)
                                                <div
                                                    class="bg-white p-2 rounded border text-center d-inline-block shadow-sm">
                                                    {!! $barcode !!}
                                        ...
                                        <td>
                                            @if ($item->store_id == null)
                                                <span
                                                    class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-2 py-1 rounded-pill fw-bold">Warehouse</span>
                                            @else
                                                <span
                                                    class="badge bg-warning bg-opacity-10 text-warning border border-warning border-opacity-25 px-2 py-1 rounded-pill fw-bold">Store</span>
                                            @endif
                                        </td>
                                        <td>
                                            <span
                                        ...
                                        <td class="text-center">
                                            @if ($item->store_id == null)
                                                <span class="badge bg-light text-muted border px-2 py-1 rounded-pill"><i
                                                        class="mdi mdi-lock me-1"></i>Global</span>
                                            @else
                                                <div
                                                    class="form-check form-switch d-flex justify-content-center m-0 fs-5">
                                            ...
                                            <div class="d-flex justify-content-end gap-1">
                                                ...
                                                @if (Auth::user()->hasPermission('view_stock_history'))
                                                    <a href="{{ route('inventory.history', $item->id) }}"
                                                        ...
                                                        data-bs-toggle="tooltip" title="View Stock History">
                                                        <i class="mdi mdi-history fs-6"></i>
                                                    </a>
                                                @endif

                                                <a href="{{ route('store.products.location-inventory', $item->id) }}"
                                                    class="btn btn-sm btn-light border shadow-sm text-dark"
                                                    data-bs-toggle="tooltip" title="Location Inventory">
                                                    <i class="mdi mdi-map-marker-radius fs-6"></i>
                                                </a>

                                                @if (Auth::user()->hasPermission('view_analytics'))
                                                    <a href="{{ route('store.products.analytics', $item->id) }}"
                                                        class="btn btn-sm btn-light border shadow-sm text-primary"
                                                        data-bs-toggle="tooltip" title="Analytics">
                                                ...
                                                @if ($item->store_id != null)
                                                    @if (Auth::user()->hasPermission('edit_product'))
                                                        <a href="{{ route('store.products.edit', $item->id) }}"
                                                    ...
                                                    @if (Auth::user()->hasPermission('delete_product'))
                                                        <form
                                                            action="{{ route('store.products.destroy', $item->id) }}"
                                                            method="POST" class="d-inline delete-form">
                                                            @csrf @method('DELETE')
                                                            <button type="button"
                                                                class="btn btn-sm btn-light border shadow-sm text-danger delete-btn"
                                                ...
                                                @else
                                                    @if (Auth::user()->hasPermission('edit_product'))
                                                        <a href="{{ route('store.products.edit', $item->id) }}"
                                                            class="btn btn-sm btn-light border shadow-sm text-secondary"
                                                            data-bs-toggle="tooltip" title="Edit Selling Price">
                                                            <i class="mdi mdi-pencil fs-6"></i>
                                                        </a>
                                                    @else
                                                        <button disabled
                                                            class="btn btn-sm btn-light border shadow-sm text-muted opacity-50">
                                                    ...
                                                    <button disabled
                                                        class="btn btn-sm btn-light border shadow-sm text-muted opacity-50"
                                                        data-bs-toggle="tooltip" title="Cannot delete Global Product">
                                                        <i class="mdi mdi-lock fs-6"></i>
                                                    </button>
                                                @endif
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            ...
                                            <p class="text-muted small mb-0">Try adjusting your search filters or add a
                                                new product.</p>
                                        </td>
                                    </tr>
                                @endforelse
