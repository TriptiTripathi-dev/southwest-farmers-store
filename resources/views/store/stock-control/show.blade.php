                                    ...
                                    @foreach ($request->items as $index => $item)
                                        <tr>
                                            ...
                                            @if ($request->status != 'pending')
                                                ...
                                                <td class="text-center">
                                                    <span
                                                        class="badge {{ $item->received_quantity >= ($item->dispatched_quantity ?? 0) ? 'bg-success' : 'bg-warning' }} bg-opacity-10 {{ $item->received_quantity >= ($item->dispatched_quantity ?? 0) ? 'text-success' : 'text-warning' }} px-3">
                                ...
                                if (data.success) {
                                    Swal.fire('Cancelled!', data.message, 'success').then(() => {
                                        window.location.href =
                                            "{{ route('store.stock-control.requests') }}";
                                    });
                                } else {
