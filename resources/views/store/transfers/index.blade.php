                        ...
                        <div class="col-12 col-md-4">
                            ...
                            <select name="from_store_id" id="storeSelect" class="form-select border"
                                ...
                                @foreach ($stores as $store)
                                    <option value="{{ $store->id }}">{{ $store->store_name }} ({{ $store->city }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                                ...
                                <tbody>
                                    @forelse($outgoing as $t)
                                        <tr class="align-middle border-bottom">
                                            <td class="px-3 py-3">
                                                <span
                                                    class="badge bg-light text-dark fw-bold">#{{ $t->transfer_number }}</span>
                                            </td>
                                            <td class="px-3 py-3">
                                            ...
                                            <td class="px-3 py-3">
                                                <div class="text-truncate"
                                                    title="{{ $t->product->product_name ?? 'N/A' }}">
                                                    {{ $t->product->product_name ?? 'N/A' }}
                                                </div>
                                                <small class="text-muted">{{ $t->product->upc ?? '' }}</small>
                                            </td>
                                                ...
                                                @if ($t->status == 'pending')
                                                    @if (Auth::user()->hasPermission('dispatch_transfer'))
                                                        ...
                                                        <form action="{{ route('transfers.dispatch', $t->id) }}"
                                                            method="POST" class="d-inline dispatch-form">
                                                            @csrf
                                                            <button type="button"
                                                                class="btn btn-sm btn-success d-flex align-items-center gap-1 dispatch-btn">
                                        ...
                                        <tr class="align-middle border-bottom">
                                            ...
                                            <td class="px-3 py-3">
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <span
                                                        class="fw-500">{{ $t->fromStore->store_name ?? 'N/A' }}</span>
                                                </div>
                                            </td>
                                            <td class="px-3 py-3">
                                                <div class="text-truncate"
                                            ...
                                            <td class="px-3 py-3 text-end">
                                                <span
                                                    class="badge bg-info-subtle text-info fw-bold">{{ $t->quantity_sent }}</span>
                                            </td>
                                            <td class="px-3 py-3 text-center">
                                                @if ($t->status == 'dispatched')
                                                    @if (Auth::user()->hasPermission('receive_transfer'))
                                                        <button type="button"
                                                            ...
                                                            onclick="openReceiveModal('{{ $t->id }}', '{{ $t->product->product_name }}', {{ $t->quantity_sent }})">
                                                ...
                                                @elseif($t->status == 'pending')
                                                    <span
                                                        class="badge bg-warning text-dark d-flex align-items-center justify-content-center gap-1">
                                                        <i class="fas fa-clock fa-xs"></i>
                                                        <span class="d-none d-md-inline">Pending</span>
                                                    </span>
                                                @else
                                                    <span
                                                        class="badge bg-success d-flex align-items-center justify-content-center gap-1">
            ...
            <form id="receiveForm" method="POST" class="modal-content border-0 shadow">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="mb-4">
                        ...
                        <p class="bg-light p-3 rounded-2 mb-0">
                            <strong id="modalExpectedQty" class="text-info"></strong> <span
                                class="text-muted">units</span>
                        </p>
                    </div>
                    ...
                    <div>
                        <div class="d-flex align-items-center gap-2 mb-2">
                            ...
                            <label for="modalReceivedQty" class="fw-bold text-dark mb-0">Actual Received
                                Quantity</label>
                        </div>
